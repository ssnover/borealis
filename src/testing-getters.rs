...
fn main() {
    ...
    .unwrap();
    println!("Firmware Version: {}", aurora.get_firmware_version().unwrap());
    ...
    for effect in aurora.get_effects().unwrap() {
        println!("Changing to effect: {}", &effect);
        aurora.set_effect(effect);
        std::thread::sleep(std::time::Duration::from_secs(10));
    }

    aurora.set_effect("Forest".to_string());
}
